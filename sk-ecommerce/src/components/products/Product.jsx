...
export function Product({ id, name, price, image, stars, sold }) {
    ...
    return (
        <div className="w-[200px] hover:cursor-pointer" onClick={() => navigate(`/products/${id}`)}>
            {
                loading && (
            ...
            <div className={`${loading && 'hidden'} w-[200px] h-[290px] flex flex-col justify-between`}>
                {
                    imageError && (
                        <div className="w-[200px] h-[200px] rounded-2xl flex justify-center items-center bg-[#eaebef]">
                            <img src={warningPNG} className="w-24 h-24" />
                        </div>
                    )
                }
                <img
                ...
                <div className="w-[120px] h-6 flex justify-between items-center">
                    <FontAwesomeIcon icon={faStarHalfStroke} className="text-slate-800" />
                    <p className="text-slate-800 text-sm">{stars}</p>
                    <p className="text-slate-800 font-light">|</p>
                    <p className="rounded-md bg-[#eaebef] text-slate-800 text-[12px] px-2 py-0.5">{sold}</p>
                </div>
